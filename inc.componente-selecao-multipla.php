...
if(($_SESSION['permissao'][$ttabela]['ids_'.$cclasse]['cadastrar'] && !$_GET['editar']) || (($_SESSION['permissao'][$ttabela]['ids_'.$cclasse]['editar'] || $_SESSION['permissao'][$ttabela]['ids_'.$cclasse]['visualizar']) && $_GET['editar'])){ ?>
<?php if(!$_SESSION['ids_'.$cclasse.'_usuarios'] && (${$ttabela}['ids_'.$cclasse] || !$_SESSION['permissao'][$ttabela]['ids_'.$cclasse]['ocultar'] || $_SESSION['pai'] == $cclasse || $_SESSION['ids_pperfis'] == 1)){
...
?>
<option value="<?php echo $classe['id'] ?>" <?php if(in_array($classe['id'], explode(', ', ${$ttabela}['ids_'.$cclasse]))){ ?>selected<?php } ?> style="white-space: normal;"><?php echo $classe[$ppropriedade] ?></option>
<?php
}
?>
</select>
