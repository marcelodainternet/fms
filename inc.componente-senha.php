...
<span class="input-group-addon" id="basic-addon2"><i class="fas fa-lock"></i></span>
<input type="password" autocomplete="<?php echo rand(); ?>" name="<?php echo $ppropriedade ?>" <?php if($_SESSION['permissao'][$ttabela][$ppropriedade]['obrigatorio']){ ?>required="required" <?php } ?>class="form-control" id="<?php echo $ppropriedade ?>" <?php if(!$_SESSION['permissao'][$ttabela][$ppropriedade]['editar'] && $_GET['editar']){ ?>disabled <?php } ?> <?php echo implode_if_is_array(' ', $pparametros); ?>/>
</div>
</div>
</div>
<?php
