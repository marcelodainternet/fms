...
<span class="input-group-addon"><i class="far fa-calendar-alt"></i> <i class="far fa-clock"></i></span>
<input type="datetime-local" <?php if($_SESSION['permissao'][$ttabela][$ppropriedade]['obrigatorio']){ ?>required="required" <?php } ?>name="<?php echo $ppropriedade ?>" id="<?php echo $ppropriedade ?>" class="form-control" value="<?php if(!$_GET['editar']){ ${$ttabela}[$ppropriedade] = date('Y-m-d H:i'); } echo str_replace(' ', 'T', ${$ttabela}[$ppropriedade]); ?>" maxlength="16" <?php if(!$_SESSION['permissao'][$ttabela][$ppropriedade]['editar'] && $_GET['editar']){ ?>disabled <?php } ?> autocomplete="<?php echo rand(); ?>" <?php echo implode_if_is_array(' ', $pparametros); ?>/>
</div>
</div>
</div>
<?php
