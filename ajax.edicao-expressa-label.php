...
$label = sql($a, 'SELECT label AS valor FROM '.ai($a, $_GET['campo']).' WHERE id = \''.ai($a, $_GET['valor']).'\'');
echo '<span class="label label-'.$label.'">'.$valor.'</span>';
...
$pparametros[] = 'onblur="salva_edicao_expressa_label(\''.strip_tags($_GET['tabela']).'\',\''.strip_tags($_GET['campo']).'\',\''.strip_tags($_GET['componente']).'\','.strip_tags($_GET['id']).',this.value,\''.strip_tags($_GET['nome']).'\',\''.strip_tags($_GET['primeiro_campo']).'\');"';
