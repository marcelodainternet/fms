...
<input autocomplete="<?php echo rand(); ?>" name="<?php echo $ppropriedade ?>" type="text" <?php if($_SESSION['permissao'][$ttabela][$ppropriedade]['obrigatorio']){ ?>required="required" <?php } ?>class="form-control" id="<?php echo $ppropriedade ?>" value="<?php if(${$ttabela}[$ppropriedade] > 0){ echo number_format(${$ttabela}[$ppropriedade], 2, ',', '.'); } ?>" onkeyup="mascara_moeda(this);" maxlength="20" <?php if(!$_SESSION['permissao'][$ttabela][$ppropriedade]['editar'] && $_GET['editar']){ ?>disabled <?php } ?> <?php echo implode_if_is_array(' ', $pparametros); ?>/>
</div>
</div>
</div>
<?php
