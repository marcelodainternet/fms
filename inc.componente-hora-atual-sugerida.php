...
<input type="time" <?php if($_SESSION['permissao'][$ttabela][$ppropriedade]['obrigatorio']){ ?>required="required" <?php } ?>name="<?php echo $ppropriedade ?>" id="<?php echo $ppropriedade ?>" class="form-control" value="<?php if(!$_GET['editar']){ ${$ttabela}[$ppropriedade] = date('H:i'); } echo ${$ttabela}[$ppropriedade]; ?>" maxlength="5" <?php if(!$_SESSION['permissao'][$ttabela][$ppropriedade]['editar'] && $_GET['editar']){ ?>disabled <?php } ?> autocomplete="<?php echo rand(); ?>" <?php echo implode_if_is_array(' ', $pparametros); ?>/>
</div>
</div>
</div>
<?php
